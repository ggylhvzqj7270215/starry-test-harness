...
fn run_file_io_basic() -> Result<(), String> {
    let file_path = temp_file("starry-file-io", true).map_err(|e| e.to_string())?;
    let initial_payload = b"starry file io smoke test";
    write_bytes(&file_path, initial_payload).map_err(|e| e.to_string())?;
    let append_payload = b" + append";
    append_bytes(&file_path, append_payload).map_err(|e| e.to_string())?;

    let mut expected = initial_payload.to_vec();
    expected.extend_from_slice(append_payload);

    let read_back = read_bytes(&file_path).map_err(|e| e.to_string())?;
    cleanup_file(&file_path).map_err(|e| e.to_string())?;
